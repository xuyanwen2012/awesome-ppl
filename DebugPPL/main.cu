...
void test_binary_radix_tree(const int grid_size)
{
	...
	for (auto i = 0; i < cpu_n_brt_nodes; i++)
	{
		EXPECT_EQ(cpu_tree.u_prefix_n[i], gpu_pip->brt.u_prefix_n[i]) << "Mismatch at index " << i;
...
void test_edge_count(const int grid_size)
{
	...
	for (auto i = 0; i < cpu_n_brt_nodes; i++)
	{
		shared::process_edge_count_i(i, gpu_pip->brt.u_prefix_n, gpu_pip->brt.u_parents, cpu_edge_count.data());
	}
	std::exclusive_scan(cpu_edge_count.begin(), cpu_edge_count.end(), cpu_edge_offset.begin(), 0);

	for (auto i = 0; i < cpu_n_brt_nodes; i++)
	{
		ASSERT_EQ(cpu_edge_count[i], gpu_pip->u_edge_counts[i]) << "Mismatch at index " << i;
		ASSERT_EQ(cpu_edge_offset[i], gpu_pip->u_edge_offsets[i]) << "Mismatch at index " << i;
	}
}
...
void cpu_build_octree(const pipe* p, const int n_brt_nodes)
{
	const auto root_level = p->brt.u_prefix_n[0] / 3;
	const auto root_prefix = p->getSortedKeys()[0] >> (morton_bits - (3 * root_level));
...
void test_octree(const int grid_size)
{
	...
	gpu::dispatch_RemoveDuplicates_sync(test_case_grid_size, stream_id, *gpu_pip);
	gpu::dispatch_BuildRadixTree(test_case_grid_size, stream_id, *gpu_pip);
	gpu::dispatch_EdgeCount(test_case_grid_size, stream_id, *gpu_pip);
	gpu::dispatch_EdgeOffset(test_case_grid_size, stream_id, *gpu_pip);
	SYNC_DEVICE();


	// make a copy of the pipe (since the seed is the same, the result should be the same)
	auto cpu_pip = std::make_unique<pipe>(n, min_coord, range, seed);
	std::copy_n(gpu_pip->u_points, n, cpu_pip->u_points);
	gpu::dispatch_ComputeMorton(test_case_grid_size, stream_id, *cpu_pip);
	gpu::dispatch_RadixSort(test_case_grid_size, stream_id, *cpu_pip);
	gpu::dispatch_RemoveDuplicates_sync(test_case_grid_size, stream_id, *cpu_pip);
	gpu::dispatch_BuildRadixTree(test_case_grid_size, stream_id, *cpu_pip);
	gpu::dispatch_EdgeCount(test_case_grid_size, stream_id, *cpu_pip);
	gpu::dispatch_EdgeOffset(test_case_grid_size, stream_id, *cpu_pip);
	SYNC_DEVICE();
...
TEST(TestOctree, GridSize)
{
	EXPECT_NO_FATAL_FAILURE(test_octree(8));
}
